---
week: 6
page: 3
title: Making progress under uncertainty
created: 2026-04-01
last_updated: 2026-04-02
status: draft
competencies:
  - area: Critical Thinking
    subskill: Prioritization
  - area: Critical Thinking
    subskill: Problem Framing
---

# Making progress under uncertainty

In professional work, production rarely starts with everything you need. The skill isn't waiting for certainty — it's knowing the difference between a gap that blocks you and a gap you can build around. How you handle that difference is one of the more visible things a team member demonstrates during a project.

[VIDEO: Making progress under uncertainty]

The video above walks through three scenarios showing what stalling looks like versus what moving forward looks like. The framework below is the reference version of the same distinctions.

---

## Three types of ambiguity (and why they require different responses)

Not all uncertainty is the same, and treating different kinds the same way leads to the wrong response. When production stalls, it's usually because of one of three things.

**Missing information**

A specific fact, data point, or input you genuinely don't have. The gap is real, and you can name it precisely.

What to do: targeted research to fill it, a specific clarifying question to a teammate or your team's employer contact, or a documented flag that acknowledges the gap and states what you assumed in its absence. The key word is *targeted* — a clearly named gap has a clearly named fix.

What not to do: treat a missing piece of supporting evidence as a reason to stop drafting entirely. Most missing information is gap-filling, not structural. You can draft around it and return.

**Unclear expectations**

You have the material, but you're not certain what "done" looks like. This is usually a question about standard or direction, not about facts.

What to do: go back to the project brief, the deliverable rubric, or the employer's framing from the Direction Check. Look for comparable examples — what did a strong version of this kind of section look like elsewhere? In most cases, unclear expectations are resolvable without asking anyone. If they're not, a specific question to your team ("I read the brief as asking for X — does that match your reading?") is better than a general one.

**Conflicting priorities**

Different inputs — research findings, employer feedback, team assumptions — point in different directions, and you're not sure which one to follow.

What to do: name the tension explicitly, to yourself and to your team. "The employer mentioned X in the Direction Check, but our research suggests Y — how do we want to handle that?" Most teams discover that naming the conflict is most of the work. Once it's on the table, deciding what to do is usually straightforward.

---

## When to keep going vs. when to stop

These aren't two equally weighted options. In most production situations, keeping going is the right default. The question is when it isn't.

**Move forward when:**

- You can draft the section and fill the gap later without restructuring what you've written
- The missing piece is supporting detail, not the core claim or direction of your section
- You can make a documented assumption and note it clearly in your draft ("Assuming X, which we'll verify by...") so that a teammate reviewing it knows what to check

**Pause when:**

- The gap is structural — if you got the missing information and it contradicted your current direction, you'd have to start over
- You're genuinely blocked waiting on a teammate to deliver something you need before you can proceed — this is a coordination problem, not a writing problem, and it needs to be named as such
- You've spent an hour researching and you're not learning anything new — at that point you likely have enough to draft, and more research is displacement activity

**Flagging a blocker without stalling the team**

When you're genuinely blocked, the way you flag it matters. Be specific: "I'm waiting on [specific thing] before I can complete [specific part of my section]" is actionable. "I'm stuck" is not. Flag it in your team's shared channel, and then move to a different part of your section while you wait. Almost every section has parts you can work on in parallel.

---

## Going further

[How well do you handle ambiguity on a project?](https://blueskiesconsulting.com/how-well-do-you-handle-ambiguity-on-a-project/) — Blue Skies Consulting
