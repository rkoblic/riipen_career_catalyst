...
```
Design Doc → Development Plan → Content → Videos & Interactives → HTML → Canvas Pages
```
...
| 4. Video Production   | Video script files                           | `.mp4` files + `.srt` caption files    | (Lumen5 or equivalent)                                                  |
| 5. HTML Production    | Markdown content files                       | `.html` files with transcripts         | [Canvas HTML Guide](canvas-html-guide.md)                               |
| 6. Canvas Page Build  | HTML files + videos + captions               | Live Canvas pages                      | This guide (Phase 6 below)                                              |
...
## Phase 5: Produce the HTML

**Input:** Markdown content files from Phase 2
...
3. For `[VIDEO:]` placeholders, add an HTML comment and a transcript section:
   ...
   ```
4. For `[INTERACTIVE:]` placeholders, use the interactive placeholder component
5. Review: count opening and closing `<div>` tags — they must match

**Checkpoint:** Every `.md` content file has a corresponding `.html` file. Transcripts are included for all videos. No broken `<div>` nesting.

---

## Phase 6: Build the Canvas Pages
...
### 6.1 Upload Media to Course Files
...
weekly_content/week[NN]/
  ...
  [video-name].mp4                    ← Phase 4 output (video file)
  [video-name]-captions.srt           ← Phase 4 output (caption file)
  page[N]-[short-title].html          ← Phase 5 output (Canvas-ready HTML)
...
guides/
  ...
  canvas-html-guide.md                 ← Phase 5 guide
  canvas-build-guide.md                ← This file (master workflow + Phase 6)
```
